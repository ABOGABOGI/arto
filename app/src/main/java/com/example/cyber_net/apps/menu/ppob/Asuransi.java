...
public class Asuransi extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        getSupportActionBar().setTitle("BPJS Kesehatan");
        getSupportActionBar().setSubtitle("Bayar BPJS Kesehatan");
    ...
    private void inquiry(){
        ...
        Call<Inquiry> call = apiInterface.ppob_inquiry("BPJSVAKK",nomor);
        call.enqueue(new Callback<Inquiry>() {
            ...
            public void onResponse(Call<Inquiry> call, Response<Inquiry> response) {
                ...
                if(reffid==null){
                    Toast.makeText(Asuransi.this,"Nomor Pelanggan Tidak Ditemukan",Toast.LENGTH_LONG).show();
                }else{
                    String jumlah = response.body().getJumlah();
                    String customer = response.body().getCustomer();
                    String transaksi = response.body().getNomor();
                    Intent intent = new Intent(Asuransi.this, Detail_ppob.class);
                    intent.putExtra("nama",nama);
                    intent.putExtra("reffid",reffid);
                    intent.putExtra("jumlah",jumlah);
                    intent.putExtra("produk","BPJSVAKK");
