...
public class SpinnerListAdapter2 extends ArrayAdapter<String> {
    private final LayoutInflater mInflater;
    private final Context mContext;
    private final int mResource;
    List<Kategori_pulsa.Datum> list_ppob;

    public SpinnerListAdapter2(@NonNull Context context, @LayoutRes int resource,
                              @NonNull List obList) {
    ...
    private View createItemView(final int position, final View convertView, ViewGroup parent){
        ...
        keterangan.setText(list_ppob.get(position).getNama());
        id.setText(list_ppob.get(position).getId().toString());
        return view;
    }
}
