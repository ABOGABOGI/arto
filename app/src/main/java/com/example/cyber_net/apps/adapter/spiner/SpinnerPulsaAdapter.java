...
public class SpinnerPulsaAdapter extends ArrayAdapter<String> {
    private final LayoutInflater mInflater;
    private final Context mContext;
    private final int mResource;
    List<Pulsa.Datum> list_ppob;

    public SpinnerPulsaAdapter(@NonNull Context context, @LayoutRes int resource,
                               @NonNull List obList) {
    ...
    private View createItemView(final int position, final View convertView, ViewGroup parent){
        ...
        TextView harga = (TextView) view.findViewById(R.id.one_item_harga);
        keterangan.setText(list_ppob.get(position).getKeterangan());
        id.setText(list_ppob.get(position).getKode_pulsa().toString());
        harga.setText(NumberFormat.getInstance().format(list_ppob.get(position).getHarga_jual()).toString());
        return view;
    }
}
